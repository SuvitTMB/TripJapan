  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
		...
		if(doc.data().ConfirmTrip==0) {
			...
			str += '<br>'+sessionStorage.getItem("EmpZone")+' '+sessionStorage.getItem("EmpRH")+'';
			str += '</div>';

			if(doc.data().ConfirmTrip==1) {
				str += '<div class="btn-t8" style="margin:15px auto 5px auto;">ยืนยันเข้าร่วมทริป ฮอกไกโด</div>';
				str += '<div style="font-size:11px; color:#f68b1f; font-weight: 600px;">ทำรายการเมื่อ : '+doc.data().DateConfirm+'</div>';

				//str += '<div class="btn-t8" style="margin:15px 0;">คุณได้เลือก<br>ยืนยันเข้าร่วมทริป<br>เรียบร้อยแล้ว</div>';
				str += '<div class="text-notrip" style="font-size: 11px; color:#002d63;text-align:left;"><font color="#ff0000">โดยมีเงื่อนไขดังนี้</font><br><br>';
				str += '<ul style="margin-left:-30px;">';
				str += '<li>ทริปท่องเที่ยวประเทศญี่ปุ่น จำนวน 5 วัน 3 คืน กำหนดเดินทางช่วงเดือนมกราคม - กุมภาพันธ์ 2566 โดยเมืองที่ไปคือ ฮอกไกโด (โดยจะมีการประกวดราคาบริษัททัวร์ใหม่อีกครั้ง) ทั้งนี้ยึดประโยชน์ของผู้เดินทางเป็นปัจจัยสูงสุด โดยการจัดได้ จะยึดจาก Conditionขั้นต่ำ ดังต่อไปนี้ direct flight, 5 วัน 3 คืน โรงแรม 3 ดาวขึ้นไป อาหารทุกมื้อ  ( หากหลังจากนั้นยังจัดไม่ได้ภายใต้ condition และระยะเวลาดังกล่าว  ขอสงวนสิทธิ์ยกเลิกการจัดทริป และทุกท่านจะได้รับเป็นเงินสด 100% (50,000 บาท)</li>';
				...
				str += '<li>กรณียืนยันการเดินทางแล้ว จะไม่สามารถเปลี่ยนแปลงขอรับเป็นเงินสดได้ในภายหลังทุกกรณี</li>';
				str += '<li>ภายหลังยืนยันการเข้าร่วมแล้ว กรณีไม่สามารถเดินทางได้เนื่องจากเหตุสุดวิสัยในช่วงก่อนเดินทาง เช่น ติดโควิด-19 , ติดภารกิจสำคัญ , ตั้งครรภ์ ขอสงวนสิทธิ์การเปลี่ยนแปลงก่อนการเดินทาง โดยจะไม่สามารถรับเงินคืนเป็นเงินสดและไม่สามารถเปลี่ยนผู้เดินทางได้</li>';
				str += '</ul></div>';
				//str += '<div style="font-size:11px; color:#f68b1f; font-weight: 600px;">ทำรายการเมื่อ : '+doc.data().DateConfirm+'</div>';
/*
...
*/
				...
				str += '<div class="btn-t7" style="margin:15px auto 5px auto;">ยืนยันไม่เข้าร่วมทริป<br>และขอรับรางวัลเป็นเงินสด 100%</div>';
				str += '<div style="font-size:11px; color:#f68b1f; font-weight: 600px;">ทำรายการเมื่อ : '+doc.data().DateConfirm+'</div>';
				str += '<div class="text-notrip" style="font-size: 11px; color:#002d63;text-align:left;"><font color="#ff0000">โดยมีเงื่อนไขดังนี้</font><br><br>';
				str += '<ul style="margin-left:-30px;">';
	          	str += '<li>ผู้ได้รับรางวัลจะได้รับเป็นเงินสด 100% ของมูลค่าทริปตั้งแต่ที่นั่งแรก (50,000 บาท)</li>';
	          	str += '<li>เงินรางวัลจะโอนเข้าบัญชีพนักงานในรอบเดือนพฤศจิกายน 2565</li>';
	          	str += '<li>พรูเด็นเชียลประกันชีวิตจะเป็นผู้รับผิดชอบภาษีเงินได้ ตามฐานเงินเดือนของพนักงาน</li>';
	          	str += '<li>กรณียืนยันการรับเป็นเงินสด 100% แล้ว จะไม่สามารถเปลี่ยนแปลงขอเข้าร่วมเดินทางได้ในภายหลังทุกกรณี</li>';
				str += '</ul></div>';
			}
			document.getElementById('Loading').style.display='none';
...
function SendAnswer() {
	...
	if(sSendAnswer==null) {
		...
		if(sSendAnswer==1) {
			...
			str += '<div class="btn-t8" style="margin:15px auto 5px auto;">ยืนยันเข้าร่วมทริป ฮอกไกโด</div>';
			str += '<div style="font-size:11px; color:#f68b1f; font-weight: 600px;">ทำรายการเมื่อ : '+dateString+'</div>';
...
*/
			...
			str += '<div class="btn-t9" style="margin-top:20px;cursor: none;">ยืนยันการทำรายการของคุณ</div>';
			str += '<div class="text-name"><b>'+sessionStorage.getItem("EmpName")+'</b><br>'+sessionStorage.getItem("EmpBranch")+'';
			str += '<br>'+sessionStorage.getItem("EmpZone")+' '+sessionStorage.getItem("EmpRH")+'</div>';
			str += '<div class="btn-t7" style="margin:15px auto 5px auto;">ยืนยันไม่เข้าร่วมทริป<br>และขอรับรางวัลเป็นเงินสด 100%</div>';
			str += '<div style="font-size:11px; color:#f68b1f; font-weight: 600px;">ทำรายการเมื่อ : '+dateString+'</div>';
